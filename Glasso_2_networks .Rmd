...
beta_GOLAZO_full <- matrix(NA, nrow = N, ncol = 3)
beta0_grid_max <- 3
beta0_grid_min <- -3
beta1_grid_max <- 4
beta1_grid_min <- -5
...
EBIC_grid_length <- 20
ebic_eval_optim_GOLAZO_full_unstandarised <- rep(NA, N)

R<-list()
res<-list()
Rho_hat_GOLAZO_full<-list()

time_GOLAZO_freq.start <- Sys.time()
for(k in 1:N){
  ...
  beta_optimise[[k]] <- BayesianOptimization(
    FUN = function(beta0, beta1, beta2){ebic_eval_two_networks_BayesOpt(n, R[[k]], P_pears[[k]], E_pears[[k]], beta0, beta1, beta2, ebic.gamma = ebic.gamma, edge.tol = edge.tol, tol = 1e-6)},
  ...
  beta_GOLAZO_full[k,] <- beta_optimise[[k]]$Best_Par
  
  U <- exp(beta_GOLAZO_full[k,1] + beta_GOLAZO_full[k,2]*P_pears[[k]] + beta_GOLAZO_full[k,3]*E_pears[[k]]) 
  ...
  ebic_eval_optim_GOLAZO_full_unstandarised[k] <- ebic_eval_GLASSO_unstandarised(Y = as.matrix(Y_repeats[[k]]), res = res[[k]], n, beta_GOLAZO_full[k,], ebic.gamma = ebic.gamma, edge.tol = edge.tol, tol = 1e-6)
  
  Rho_hat_GOLAZO_full[[k]] <- threshold(cov2cor(res[[k]]$K), edge.tol)
}
time_GOLAZO_freq.end <- Sys.time()
...
ebic_eval_optim_GOLAZO_full_unstandarised

n_edges<-rep(NA,N)
for (k in 1:N){
  n_edges[k]<-sum(Rho_hat_GOLAZO_full[[k]][lower.tri((Rho_hat_GOLAZO_full[[k]]))] != 0) # number of edges
}

time_GOLAZO_freq1 <- time_GOLAZO_freq.end - time_GOLAZO_freq.start
time_GOLAZO_freq <- round(time_GOLAZO_freq1/N, 3)
time_GOLAZO_freq
...
Sigma_hat_GOLAZO <- list()   #N pxp matrices where p changes according to the year
for (k in 1:N){
  Sigma_hat_GOLAZO[[k]] <- cor2cov(Rho_hat_GOLAZO_full[[k]], sigma2_vect = diag(cov(Y_repeats[[k]])))
}
...
#minimum variance
weights_minimum_variance_GOLAZO <- list()
for (k in 1:N){
  weights_minimum_variance_GOLAZO[[k]] <- minimum_variance_portfolio(Sigma_hat_GOLAZO[[k]]) 
}

for (k in 1:N){
  weights_minimum_variance_GOLAZO[[k]] <- weights_minimum_variance_GOLAZO[[k]]/sum(weights_minimum_variance_GOLAZO[[k]])
}

#mean variance
weights_mean_variance_GOLAZO <- list()
for (k in 1:N){
  weights_mean_variance_GOLAZO[[k]] <- mvPortfolio(mean[[k]], Sigma_hat_GOLAZO[[k]], min.return=baseline_portfolio[k])
}
...
#minimum variance
expected_return_portfolio_minimum_variance_GOLAZO <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_minimum_variance_GOLAZO[k] <- expected_return_portfolio_minimum_variance(Sigma_hat_GOLAZO[[k]], mean[[k]])
}

#mean variance
expected_return_portfolio_mean_variance_GOLAZO <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_mean_variance_GOLAZO[k] <- sum(weights_mean_variance_GOLAZO[[k]]*mean[[k]])
}
...
W_mk <- list()
for (k in 1:N){
  one.vec <- rep(1,nrow(Sigma_hat_GOLAZO[[k]]))
  W_mk[[k]] <- (mean[[k]]%*%solve(Sigma_hat_GOLAZO[[k]]))/(as.numeric(t(one.vec[k]) %*% mean[[k]] %*% solve(Sigma_hat_GOLAZO[[k]])))
}

v_2PS_GOLAZO <- list()
for (k in 1:N){
  v_2PS_GOLAZO[[k]] <- weights_mean_variance_GOLAZO[[k]]-weights_minimum_variance_GOLAZO[[k]]
}

alpha_2PS_GOLAZO <- rep(NA,N)
for (k in 1:N){
  alpha_2PS_GOLAZO[k] <- ((sum(mean[[k]])/p[k])-(t(mean[[k]]) %*% weights_minimum_variance_GOLAZO[[k]]))/(t(mean[[k]]) %*% v_2PS_GOLAZO[[k]])
}

w_2PS_GOLAZO <- list()
for (k in 1:N){
  w_2PS_GOLAZO[[k]] <- weights_minimum_variance_GOLAZO[[k]]+(v_2PS_GOLAZO[[k]]*alpha_2PS_GOLAZO[k])
}

expected_return_portfolio_2PS_GOLAZO <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_2PS_GOLAZO[k] <- t(mean[[k]])%*%w_2PS_GOLAZO[[k]]
}
...
#plots
for (k in 1:N){
  matrixplot(Sigma_hat_GOLAZO[[k]]-diag(NA,p[k]))
  matrixplot(Rho_hat_GOLAZO_full[[k]]-diag(NA,p[k]))
}
...
beta_GOLAZO_full_FF <- matrix(NA, nrow = N, ncol = 3)
beta0_grid_max <- 3
beta0_grid_min <- -3
beta1_grid_max <- 4
beta1_grid_min <- -5
...
EBIC_grid_length <- 20
ebic_eval_optim_GOLAZO_full_unstandarised_FF <- rep(NA, N)

R<-list()
res<-list()
Rho_hat_GOLAZO_full_FF<-list()
...
for(k in 1:N){
  ...
  beta_GOLAZO_full_FF[k,] <- beta_optimise[[k]]$Best_Par
  # JACK: Saving the EBIC 
  ebic_eval_optim_GOLAZO_full_FF[k] <- - beta_optimise[[k]]$Best_Value
  
  U <- exp(beta_GOLAZO_full_FF[k,1] + beta_GOLAZO_full_FF[k,2]*P_pears[[k]] + beta_GOLAZO_full_FF[k,3]*E_pears[[k]]) 
  ...
  ebic_eval_optim_GOLAZO_full_unstandarised_FF[k] <- ebic_eval_GLASSO_unstandarised(Y = as.matrix(Y_repeats_FF[[k]]), res = res[[k]], n, beta_GOLAZO_full_FF[k,], ebic.gamma = ebic.gamma, edge.tol = edge.tol, tol = 1e-6)
  
  Rho_hat_GOLAZO_full_FF[[k]] <- threshold(cov2cor(res[[k]]$K), edge.tol)
}
time_GOLAZO_freq.end <- Sys.time()
...
ebic_eval_optim_GOLAZO_full_unstandarised_FF

n_edges<-rep(NA,N)
for (k in 1:N){
  n_edges[k]<-sum(Rho_hat_GOLAZO_full_FF[[k]][lower.tri((Rho_hat_GOLAZO_full_FF[[k]]))] != 0) # number of edges
}
...
Sigma_hat_GOLAZO_FF <- list()
for (k in 1:N){
  Sigma_hat_GOLAZO_FF[[k]] <- cor2cov(Rho_hat_GOLAZO_full_FF[[k]], sigma2_vect = diag(cov(Y_repeats_FF[[k]])))
}
...
#minimum variance
weights_minimum_variance_GOLAZO_FF <- list()
for (k in 1:N){
  weights_minimum_variance_GOLAZO_FF[[k]] <- minimum_variance_portfolio(Sigma_hat_GOLAZO_FF[[k]]) 
}

for (k in 1:N){
  weights_minimum_variance_GOLAZO_FF[[k]] <- weights_minimum_variance_GOLAZO_FF[[k]]/sum(weights_minimum_variance_GOLAZO_FF[[k]])
}

#mean variance
weights_mean_variance_GOLAZO_FF <- list()
for (k in 1:N){
  weights_mean_variance_GOLAZO_FF[[k]] <- mvPortfolio(mean[[k]], Sigma_hat_GOLAZO_FF[[k]], min.return=baseline_portfolio[k])
}
...
#minimum variance
expected_return_portfolio_minimum_variance_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_minimum_variance_GOLAZO_FF[k] <- expected_return_portfolio_minimum_variance(Sigma_hat_GOLAZO_FF[[k]], mean[[k]])
}

#mean variance
expected_return_portfolio_mean_variance_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_mean_variance_GOLAZO_FF[k] <- sum(weights_mean_variance_GOLAZO_FF[[k]]*mean[[k]])
}
...
W_mk <- list()
for (k in 1:N){
  one.vec <- rep(1,nrow(Sigma_hat_GOLAZO_FF[[k]]))
  W_mk[[k]] <- (mean[[k]]%*%solve(Sigma_hat_GOLAZO_FF[[k]]))/(as.numeric(t(one.vec[k]) %*% mean[[k]] %*% solve(Sigma_hat_GOLAZO_FF[[k]])))
}

v_2PS_GOLAZO_FF <- list()
for (k in 1:N){
  v_2PS_GOLAZO_FF[[k]] <- weights_mean_variance_GOLAZO_FF[[k]]-weights_minimum_variance_GOLAZO_FF[[k]]
}

alpha_2PS_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  alpha_2PS_GOLAZO_FF[k] <- ((sum(mean[[k]])/p[k])-(t(mean[[k]]) %*% weights_minimum_variance_GOLAZO_FF[[k]]))/(t(mean[[k]]) %*% v_2PS_GOLAZO_FF[[k]])
}

w_2PS_GOLAZO_FF <-list()
for (k in 1:N){
  w_2PS_GOLAZO_FF[[k]] <- weights_minimum_variance_GOLAZO_FF[[k]]+(v_2PS_GOLAZO_FF[[k]]*alpha_2PS_GOLAZO_FF[k])
}

expected_return_portfolio_2PS_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  expected_return_portfolio_2PS_GOLAZO_FF[k] <- t(mean[[k]])%*%w_2PS_GOLAZO_FF[[k]]
}
...
for (k in 1:N){
  matrixplot(Sigma_hat_GOLAZO_FF[[k]]-diag(NA,p[k]))
  matrixplot(Rho_hat_GOLAZO_full_FF[[k]]-diag(NA,p[k]))
}
...
#minimum variance
portfolio_actual_return_rate_minimum_variance_GOLAZO <- rep(NA,N)
for (k in 1:N){
  portfolio_actual_return_rate_minimum_variance_GOLAZO[k] <- sum(weights_minimum_variance_GOLAZO[[k]] * r[[k]])
}

#mean variance
portfolio_actual_return_rate_mean_variance_GOLAZO <- rep(NA,N)
for (k in 1:N){
  portfolio_actual_return_rate_mean_variance_GOLAZO[k] <- sum(weights_mean_variance_GOLAZO[[k]] * r[[k]])
}

#cumulative actual portfolio return rate with minimum and mean variance 
sum(portfolio_actual_return_rate_minimum_variance_GOLAZO)
sum(portfolio_actual_return_rate_mean_variance_GOLAZO)
...
#minimum variance
portfolio_actual_return_rate_minimum_variance_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  portfolio_actual_return_rate_minimum_variance_GOLAZO_FF[k] <- sum(weights_minimum_variance_GOLAZO_FF[[k]] * r[[k]])
}

#mean variance
portfolio_actual_return_rate_mean_variance_GOLAZO_FF <- rep(NA,N)
for (k in 1:N){
  portfolio_actual_return_rate_mean_variance_GOLAZO_FF[k] <- sum(weights_mean_variance_GOLAZO_FF[[k]] * r[[k]])
}

#cumulative actual portfolio return rate with minimum and mean variance 
sum(portfolio_actual_return_rate_minimum_variance_GOLAZO_FF)
sum(portfolio_actual_return_rate_mean_variance_GOLAZO_FF)
...
#minimum variance
daily_portfolio_return_minimum_variance_GOLAZO <- list()
daily_portfolio_return_minimum_variance_GOLAZO_FF <- list()

for (k in 1:31){
  daily_portfolio_return_minimum_variance_GOLAZO[[k]] <- Return.portfolio(Y_prova[[k]][,2:(p[k]+1)],weights=unlist(weights_minimum_variance_GOLAZO[k]), verbose=TRUE)
  
  daily_portfolio_return_minimum_variance_GOLAZO_FF[[k]] <- Return.portfolio(Y_prova[[k]][,2:(p[k]+1)],weights=unlist(weights_minimum_variance_GOLAZO_FF[k]), verbose=TRUE)
}

StdDev_minimum_variance_GOLAZO <- vector()
StdDev_minimum_variance_GOLAZO_FF <- vector()

for (k in 1:31){
  StdDev_minimum_variance_GOLAZO[k] <- StdDev(daily_portfolio_return_minimum_variance_GOLAZO[[k]]$returns)
  
  StdDev_minimum_variance_GOLAZO_FF[k] <- StdDev(daily_portfolio_return_minimum_variance_GOLAZO_FF[[k]]$returns)
}

mean(StdDev_minimum_variance_GOLAZO)
mean(StdDev_minimum_variance_GOLAZO_FF)

#mean variance
daily_portfolio_return_mean_variance_GOLAZO <- list()
daily_portfolio_return_mean_variance_GOLAZO_FF <- list()

for (k in 1:31){
  daily_portfolio_return_mean_variance_GOLAZO[[k]] <- Return.portfolio(Y_prova[[k]][,2:(p[k]+1)],weights=unlist(weights_mean_variance_GOLAZO[k]), verbose=TRUE)
  
  daily_portfolio_return_mean_variance_GOLAZO_FF[[k]] <- Return.portfolio(Y_prova[[k]][,2:(p[k]+1)],weights=unlist(weights_mean_variance_GOLAZO_FF[k]), verbose=TRUE)
}

StdDev_mean_variance_GOLAZO <- vector()
StdDev_mean_variance_GOLAZO_FF <- vector()

for (k in 1:31){
  StdDev_mean_variance_GOLAZO[k] <- StdDev(daily_portfolio_return_mean_variance_GOLAZO[[k]]$returns)
  
  StdDev_mean_variance_GOLAZO_FF[k] <- StdDev(daily_portfolio_return_mean_variance_GOLAZO_FF[[k]]$returns)
}

mean(StdDev_mean_variance_GOLAZO)
mean(StdDev_mean_variance_GOLAZO_FF)
...
#minimum variance
Sharpe_ratio_minimum_variance_GOLAZO <- vector()
Sharpe_ratio_minimum_variance_GOLAZO_FF <- vector()
...
for (k in 1:N){
  Sharpe_ratio_minimum_variance_GOLAZO[k] <- SharpeRatio((Y_prova[[k]][,2:(p[k]+1),drop=FALSE]), FUN= "StdDev", weights = unlist(weights_minimum_variance_GOLAZO[k]))
  
  Sharpe_ratio_minimum_variance_GOLAZO_FF[k] <- SharpeRatio((Y_prova[[k]][,2:(p[k]+1),drop=FALSE]), FUN= "StdDev", weights = unlist(weights_minimum_variance_GOLAZO_FF[k]))
}

mean(Sharpe_ratio_minimum_variance_GOLAZO)
mean(Sharpe_ratio_minimum_variance_GOLAZO_FF)

#mean variance
Sharpe_ratio_mean_variance_GOLAZO <- vector()
Sharpe_ratio_mean_variance_GOLAZO_FF <- vector()

for (k in 1:N){
  Sharpe_ratio_mean_variance_GOLAZO[k] <- SharpeRatio((Y_prova[[k]][,2:(p[k]+1),drop=FALSE]), FUN= "StdDev", weights = unlist(weights_mean_variance_GOLAZO[k]))
  
  Sharpe_ratio_mean_variance_GOLAZO_FF[k] <- SharpeRatio((Y_prova[[k]][,2:(p[k]+1),drop=FALSE]), FUN= "StdDev", weights = unlist(weights_mean_variance_GOLAZO_FF[k]))
}

mean(Sharpe_ratio_mean_variance_GOLAZO)
mean(Sharpe_ratio_mean_variance_GOLAZO_FF)
...
#minimum variance
HHI_minimum_variance_GOLAZO <- vector()
HHI_minimum_variance_GOLAZO_FF <- vector()

for (k in 1:N){
  HHI_minimum_variance_GOLAZO[k] <- HHI(unlist(weights_minimum_variance_GOLAZO[[k]]))
  
  HHI_minimum_variance_GOLAZO_FF[k] <- HHI(unlist(weights_minimum_variance_GOLAZO_FF[[k]]))
}

mean(HHI_minimum_variance_GOLAZO)
mean(HHI_minimum_variance_GOLAZO_FF)

#mean variance
HHI_mean_variance_GOLAZO <- vector()
HHI_mean_variance_GOLAZO_FF <- vector()

for (k in 1:N){
  HHI_mean_variance_GOLAZO[k] <- HHI(unlist(weights_mean_variance_GOLAZO[[k]]))
  
  HHI_mean_variance_GOLAZO_FF[k] <- HHI(unlist(weights_mean_variance_GOLAZO_FF[[k]]))
}

mean(HHI_mean_variance_GOLAZO)
mean(HHI_mean_variance_GOLAZO_FF)
